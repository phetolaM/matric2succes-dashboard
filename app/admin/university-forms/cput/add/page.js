...
export default function AddCourseCPUTPage() {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        ...
        setSubjects(sorted);
        setLanguageSubjects(sorted.filter((s) => s.isLanguage === true));
      } catch (err) {
  ...
  const addSubjectRequirement = () => {
    setFormData((prev) => ({
      ...prev,
      subjectRequirements: [...prev.subjectRequirements, emptyGroupItem()],
    }));
  };
  ...
  const addGroupItem = (groupIdx) => {
    setFormData((prev) => ({
      ...prev,
      subjectRequirementGroups: prev.subjectRequirementGroups.map((group, i) =>
  ...
  const removeGroupItem = (groupIdx, itemIdx) => {
    setFormData((prev) => ({
      ...prev,
      subjectRequirementGroups: prev.subjectRequirementGroups.map((group, i) =>
        i === groupIdx ? group.filter((_, j) => j !== itemIdx) : group,
      ),
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cleanData = {
        ...
        apsRequirementMathematics: normInt(formData.apsRequirementMathematics),
        ...
        languageRequirementOperator: formData.languageRequirementOperator,
        languageRequirements: formData.languageRequirements
          ...
          .map((lr) => ({
            ...
            homeLanguagePercentage: lr.homeLanguagePercentage
              ...
              : undefined,
            additionalLanguagePercentage: lr.additionalLanguagePercentage
      ...
      const res = await fetch(`/api/course-collections/${UNIVERSITY_CODE}`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(
          apiError?.message || `API Error: ${res.status} ${res.statusText}`,
        );
      }
  ...
  return (
    <div className={styles.container}>
      ...
      <form onSubmit={handleSubmit} className={formStyles.form}>
        <div className={formStyles.section}>
          ...
          <div className={formStyles.fieldGroup}>
            <label className={formStyles.label}>
              Course Name <span className={formStyles.required}>*</span>
            </label>
            <input
              type="text"
            ...
            />
            {errors.courseName && (
              <p className={formStyles.error}>{errors.courseName}</p>
            )}
          </div>
          ...
          {/* Qualification Level */}
          <div className={formStyles.fieldGroup}>
            <label className={formStyles.label}>Qualification Level</label>
            <select
              value={formData.qualificationLevel}
              onChange={(e) =>
                handleChange("qualificationLevel", e.target.value)
              }
              className={formStyles.input}
              disabled={isLoading}
            >
          ...
          {/* Method of Study */}
          <div className={formStyles.fieldGroup}>
            <label className={formStyles.label}>Method of Study</label>
            <select
              value={formData.methodOfStudy}
              onChange={(e) => handleChange("methodOfStudy", e.target.value)}
              className={formStyles.input}
              disabled={isLoading}
            >
              {methodOptions.map((m) => (
          ...
          {/* APS Requirements */}
          <div className={formStyles.section}>
            <h2 className={formStyles.sectionTitle}>APS Requirements</h2>
            ...
            <div className={formStyles.fieldGroup}>
              ...
              <input
                ...
                onChange={(e) => handleChange("apsRequirement", e.target.value)}
                className={formStyles.input}
                disabled={isLoading}
                min="0"
              />
            ...
            <div className={formStyles.fieldGroup}>
              <label className={formStyles.label}>APS with Mathematics</label>
              <input
                type="number"
                value={formData.apsRequirementMathematics}
                onChange={(e) =>
                  handleChange("apsRequirementMathematics", e.target.value)
                }
                className={formStyles.input}
                disabled={isLoading}
                min="0"
            ...
            <div className={formStyles.fieldGroup}>
              <label className={formStyles.label}>APS with Math Literacy</label>
              <input
                type="number"
                value={formData.apsRequirementMathLit}
                onChange={(e) =>
                  handleChange("apsRequirementMathLit", e.target.value)
                }
                className={formStyles.input}
                disabled={isLoading}
                min="0"
            ...
            <div className={formStyles.fieldGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  handleChange("apsRequirementTechnicalMath", e.target.value)
                }
                className={formStyles.input}
                disabled={isLoading}
                min="0"
          ...
          <div className={formStyles.arrayField}>
            {formData.careerChoices.map((choice, idx) => (
              <div key={idx} className={formStyles.arrayItem}>
                <div className={formStyles.input} style={{ display: "block" }}>
                  {choice}
                </div>
                <button
                  type="button"
            ...
            <div className={formStyles.arrayItem}>
              <input
                ...
                onChange={(e) => setCareerChoiceDraft(e.target.value)}
                className={formStyles.input}
                disabled={isLoading}
              ...
              <button
                ...
                disabled={isLoading || !careerChoiceDraft.trim()}
              >
                <FiPlus /> Add
              </button>
            </div>
        ...
        <div className={formStyles.section}>
          <h2 className={formStyles.sectionTitle}>Language Requirements</h2>
          <p
            style={{
              fontSize: "0.9em",
              color: "#666",
          ...
          >
            Add languages with different percentage requirements for Home
            Language vs Additional Language
          </p>
          <div className={formStyles.arrayField}>
            {formData.languageRequirements.map((req, idx) => (
              ...
              >
                <div className={formStyles.fieldGroup}>
                  <label className={formStyles.label}>Language</label>
                  <select
                    value={req.subjectId}
                    onChange={(e) =>
                  ...
                  >
                    ...
                    {getAvailableLanguageSubjects(idx).map((s) => (
                ...
                <div className={formStyles.fieldGroup}>
                  <label className={formStyles.label}>Home Language %</label>
                  <input
                    type="number"
            ...
            {formData.subjectRequirements.map((req, idx) => (
              <div key={idx} className={formStyles.arrayItem}>
                <select
                  value={req.subjectId}
                  onChange={(e) =>
                    updateSubjectRequirement(idx, "subjectId", e.target.value)
                  }
                  className={formStyles.input}
                >
                  <option value="">-- select subject --</option>
                  {getAvailableSubjectsForAND(idx).map((s) => (
                    <option key={s._id} value={s._id}>
                      {s.name}
                    </option>
                  ))}
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateSubjectRequirement(idx, "percentage", e.target.value)
                  }
                  className={formStyles.input}
                  min="0"
                  max="100"
                ...
                <button
                  type="button"
                  onClick={() => removeSubjectRequirement(idx)}
                  className={formStyles.removeBtn}
                  disabled={isLoading}
                >
                  <FiX />
        ...
        <div className={formStyles.section}>
          ...
          {formData.subjectRequirementGroups.map((group, gIdx) => (
            ...
            >
              ...
              <div className={formStyles.arrayField}>
                {group.map((item, iIdx) => (
                  <div key={iIdx} className={formStyles.arrayItem}>
                    <select
                      value={item.subjectId}
                      onChange={(e) =>
                        updateGroupItem(gIdx, iIdx, "subjectId", e.target.value)
                      }
                      className={formStyles.input}
                    >
                      <option value="">-- select subject --</option>
                      {getAvailableSubjectsForGroup(gIdx, iIdx).map((s) => (
                        <option key={s._id} value={s._id}>
                          {s.name}
                        </option>
                      ))}

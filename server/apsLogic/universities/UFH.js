export const meta = {
    code: "ufh",
    displayName: "University of Fort Hare",
    collections: ["ufh"],
    mathSubjects: [
...
export function calculateAPS(data) {
    ...
    // Life Orientation (included)
    if (typeof data.lifeOrientationPercent === "number") {
        totalAPS += percentToAps(data.lifeOrientationPercent);
    }

    // Other Subjects (including Life Orientation if present in the list)
    (data.otherSubjects || []).forEach((subject) => {
        totalAPS += percentToAps(subject.percent || 0);
    });

    console.log(`UFH APS Score:  ${totalAPS}`);
    return { aps: totalAPS };
}

export const meta = {
    code: "ufs",
    displayName: "University of the Free State",
    collections: ["ufs"],
    mathSubjects: [
...
export function calculateAPS(data) {
    ...
    // Sort and take the best 6 academic subjects
    subjectPercents = subjectPercents
        .filter((p) => p >= 30) // Only subjects with 30% or more
        .sort((a, b) => b - a)
        .slice(0, 6);

    // Calculate AP for best 6
    let totalAP = subjectPercents.reduce(
        (sum, percent) => sum + percentToAp(percent),
        0
    );

    // Life Orientation (special rule)
    let loAP = 0;
    if (
        typeof data.lifeOrientationPercent === "number" &&
        data.lifeOrientationPercent >= 60
    ) {
        loAP = 1;
    }
    totalAP += loAP;

    console.log(`UFS APS Score:  ${totalAP}`);
    return { aps: totalAP };
}
